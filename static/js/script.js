document.addEventListener("DOMContentLoaded", function () {
    ...
    mapImage.onerror = function () {
        ...
        displayErrorMessage(
            "Failed to load map image. Please try again later.",
        );
    };
    ...
    function resizeCanvas() {
        if (mapCanvas) {
            ...
            mapCanvas.height =
                (mapImage.height / mapImage.width) * containerWidth;
    ...
    function drawMap() {
        if (ctx && mapImage.complete && mapImage.naturalHeight !== 0) {
            ...
            console.error(
                "Unable to draw map: Canvas context or image not ready",
            );
        }
    }
    ...
    function loadMaps() {
        fetch("/api/maps")
            ...
            .then((data) => {
                ...
                const iLabOption = Array.from(mapSelector.options).find(
                    (option) => option.text === "iLab",
                );
                if (iLabOption) {
    ...
    function updateItemList() {
        ...
        items.forEach((item) => {
            ...
            li.addEventListener('mouseover', () => addMarker(item));
            li.addEventListener('mouseout', () => removeMarker());
    ...
    function performSearch() {
        ...
        fetch(
            `/api/search?q=${encodeURIComponent(query)}&type=${type}&map_id=${currentMapId}`,
        )
            .then((response) => {
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
            ...
            .catch((error) => {
                ...
                displayErrorMessage(
                    "Error searching items. Please try again later.",
                );
            });
    }
    ...
    function displayErrorMessage(message) {
        ...
        errorDiv.className =
            "alert alert-danger alert-dismissible fade show notification";
        errorDiv.role = "alert";
        errorDiv.innerHTML = `
            ${message}
    ...
    function displaySuccessMessage(message) {
        ...
        successDiv.className =
            "alert alert-success alert-dismissible fade show notification";
    ...
    function showItemDetails(itemId) {
        ...
        document.getElementById("itemDetailsQuantity").textContent =
            `Quantity: ${item.quantity}`;
        document.getElementById("itemDetailsDescription").textContent =
    ...
    function updateItem(item) {
        ...
        formData.append(
            "quantity",
            parseInt(document.getElementById("itemQuantity").value, 10) || 1,
        );
        ...
        formData.append(
            "y_coord",
            (selectedLocation
                ? selectedLocation.y / scale
                : item.y_coord
            ).toString(),
        );
        // Add selected coordinates
        console.log("Submitting coordinates:", selectedCoords);
        formData.append("z_coord_model", selectedCoords.z);
        formData.append("x_coord_model", selectedCoords.x);
        formData.append("y_coord_model", selectedCoords.y);
        ...
        fetch(`/api/items/${item.id}`, {
            ...
            .then((data) => {
                ...
                loadItems();
                // Remove marker after successful submission
                // if (previousMarker) {
    ...
    function saveItem() {
        if (!selectedLocation || !currentMapId) {
            displayErrorMessage(
                "Please select a location on the map and ensure a map is selected.",
            );
            return;
        }
        ...
        const itemWarningInput = document.querySelectorAll(
            ...
            if (addItemForm.style.display === "block") {
                if (updateItemBtn.textContent === "Update Item") {
                    ...
                    if (item) {
                        ...
                        displaySuccessMessage(
                            'Item location updated. Click "Update Item" to save changes.',
                        );
                    }
                } else {
                    positionAddItemForm();
    ...
    function updateItemImage(itemId, file) {
        ...
        fetch(`/api/items/${itemId}`, {
            ...
            .catch((error) => {
                ...
                displayErrorMessage(
                    "Error updating item image. Please try again later.",
                );
            });
    }
